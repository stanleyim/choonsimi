"""
fetch_kis_flow.py  v5.2
────────────────────────────────────────────
v5.1 대비 변경:
  ✔ 토큰 재사용: kis_token.json 우선 읽기
    (fetch_data.py 가 먼저 발급한 토큰 재사용 → 403 방지)
  ✔ 토큰 없을 때만 새로 발급 (fallback 유지)
  ✔ 기존 v5.1 로직 완전 유지
"""
...
TOKEN_FILE      = "kis_token.json"   # ← v5.2 추가
...
# ─────────────────────────────
def get_token() -> str:
    """kis_token.json 우선 사용, 없으면 새로 발급"""
    try:
        with open(TOKEN_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        token = data.get("access_token", "")
        if token:
            print("[AUTH] kis_token.json 재사용")
            return token
    except Exception:
        pass
...
def _handle_no_data(key: str) -> dict:
    ...
    return {"score": prev["score"], "confidence": 0.3,
            "status": "no_data", "rows": []}


def _handle_degraded(key: str) -> dict:
    prev = _last_known.get(key, {"score": 0.5, "confidence": 0.3})
    return {"score": prev["score"], "confidence": 0.2,

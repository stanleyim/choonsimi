"""
fetch_fundamental.py — v2.4
────────────────────────────────────────────────────────────
v2.4 변경:
  ✅ opendartreader 소문자 import로 수정 (pip 패키지명 기준)
...
try:
    from opendartreader import OpenDartReader as _DartClass # ✅ 소문자 import
except ImportError:
    _DartClass = None
...
def main():
    ...
    if _DartClass is None:
        print("[ERROR] opendartreader 미설치 → requirements.txt 확인")
        _save_empty()
        return

    dart = _DartClass(api_key) # ✅ 올바른 초기화

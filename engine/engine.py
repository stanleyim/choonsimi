"""
engine/engine.py — v37.2 FINAL STABLE
수정:
  1. load_news() → news_fetch.run() 직접 호출 (뉴스 실시간 반영)
  2. history.csv 중복 방지 (date+code 기준)
  3. 중복 컬럼으로 인한 InvalidIndexError 해결
  4. [FIX] result.json top10에 name 추가 (UI 종목명 표시)
"""
...
# =========================
def run():
    print("[ENGINE v37.2 FINAL START]")
    ...
    result = {
        ...
        "regime": regime,
        # ★ name 추가 (UI 종목명 표시)
        "top10":  top[["code", "name", "close", "score"]].round(4).to_dict("records"),
    }

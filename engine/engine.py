"""
RegimeEngine v6.4.0 — FINAL PRODUCTION HARDENED (STABLE+)
─────────────────────────────────────────────────────
...
✔ FIX: verify() evaluated-only denominator + 7-day gap handling
✔ FIX: score scaling consistency (0–100 stable mapping)
✔ ADD: stability smoothing (top20 continuity 4% boost)
✔ SAFETY: KR holiday support + full defensive structure
─────────────────────────────────────────────────────
"""
...
class RegimeEngine:
    ...
    def run(self):
        ...
        print("[DONE]", result.get("regime"), len(result.get("top20", [])),
              f"| Stability boost: {len(prev_top_codes)} 종목")

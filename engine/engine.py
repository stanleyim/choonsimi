    ...
    def score(self, stock: dict) -> float:
        ...
        raw = (
            ...
            (calc_fundamental_score(
                safe_float(fd.get("roe")),
                safe_float(fd.get("debt_ratio")),
                safe_float(fd.get("op_growth"))
            ) if fd else 0.5) * W_FUNDAMENTAL +
...
class RegimeEngine:
    ...
    def _load_cache(self):
        try:
            with open(REGIME_CACHE, "r", encoding="utf-8-sig") as f:
                c = json.load(f)
    ...
    def load_market_flow(self) -> dict:
        try:
            with open("market_flow.json", "r", encoding="utf-8-sig") as f:
                return json.load(f)
        except:
    ...
    def load_news_scores(self) -> dict:
        try:
            with open("news_scores.json", "r", encoding="utf-8-sig") as f:
                return json.load(f).get("scores", {})
        except:
            print("[WARN] news_scores.json 로드 실패 → 중립 처리")
            return {}
    ...
    def load_stock_data(self) -> list:
        try:
            ...
            df_today["change_rate"] = pd.to_numeric(df_today["change_rate"], errors="coerce").fillna(0.0)
    ...
    def load_fundamental(self) -> dict:
        try:
            with open("fundamental.json", "r", encoding="utf-8-sig") as f:
                raw = json.load(f)
    ...
    def compute_flow_score(self, flow: dict) -> float:
        ...
        if history:
            ...
            s = flow.get("scores", {})
            kp_f = s.get("KOSPI_foreign", 0)
            kp_i = s.get("KOSPI_institution", 0)
            kq_f = s.get("KOSDAQ_foreign", 0)
            kq_i = s.get("KOSDAQ_institution", 0)
    ...
    def compute_regime(self, flow: float) -> dict:
        ...
        base = min(0.95, 0.5 + abs(final) * 0.6)
        penalty = min(0.15, vol * 0.3)
    ...
    def save_result(self, res: dict, flow: dict, top_n: list):
        ...
        for item in top_n:
            print(f"  {item['rank']:>2}. {item['code']} {item['name']:<12} score={item['score']:.1f} | {item['reason']}")
    ...
    def save_signal_history(self, top_n: list, regime: str):
        ...
        rows = [{
            ...
            "flow_net": i["flow_net"],
            "roe": i.get("roe", ""),
            "debt_ratio": i.get("debt_ratio", ""),
            "op_growth": i.get("op_growth", "")
        } for i in top_n]

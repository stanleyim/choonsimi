...
def safe_float(v):
    """NumPy 2.x safe scalar converter"""
    if pd.isna(v):
        return 0.0
    try:
        return float(v)
...
def compute_flow(df: pd.DataFrame) -> pd.DataFrame:
    df["foreign_net"] = df.get("foreign_net", 0).fillna(0.0)
    df["inst_net"] = df.get("inst_net", 0).fillna(0.0)
...
def compute_momentum(df: pd.DataFrame, hist: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in latest.iterrows():
        mom_map[r["code"]] = (
            0.5 * safe_float(r["ret_1"]) +
            0.3 * safe_float(r["ret_5"]) +
            0.2 * safe_float(r["ret_10"])
        )
...
def compute_dart(df: pd.DataFrame) -> pd.DataFrame:
    df["dart_score"] = df.get("dart_score", 0).fillna(0.0)
...
def apply_vol(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["weight"] = df["adj_score"] / total if total != 0 else 1.0 / len(df)

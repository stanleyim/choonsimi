...
HISTORY_PATH = "history.csv" # v7.4 추가
...
def features(s):
    ...
    return momentum, liquidity, risk, close # close 추가
...
# ─────────────────────────────
def main():
    print("[ENGINE v7.4 HISTORY START]")
    ...
    for i, code in enumerate(universe, 1):
        ...
        m, l, r, close = features(s) # close 받아오기
        sc = score(m, l, r, i)

        results.append({
            "code": code,
            "score": round(sc, 4),
            "close": close # history용
        })
    ...
    # history.csv 누적 저장 - v7.4 핵심
    with open(HISTORY_PATH, "a") as f:
        for item in results:
            f.write(f'{item["code"]},{used_date},{item["close"]},{item["score"]}\n')

    print(f"[DONE] {len(results)} stocks + history saved")

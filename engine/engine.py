"""
engine.py — v24.0 FINAL (data.json 기반 / 완전 안정)
...
def load_data():
    ...
    df["code"] = df["code"].astype(str).str.zfill(6) # 6자리 통일
...
def compute_score(df):
    ...
    if "dart_score" not in df.columns: # ← KeyError 방지
        df["dart_score"] = 0.0
    else:
...
def update_history(df, date_str):
    ...
    if os.path.exists(HISTORY_PATH):
        ...
        hist = pd.concat([hist, latest], ignore_index=True).drop_duplicates(["code", "date"])
    else:
        hist = latest
...
def compute_ic_series(hist, window=IC_WINDOW):
    ...
    h = hist.sort_values(["code", "date"]).copy()
    h["code"] = h["code"].astype(str).str.zfill(6)
    h["next_return"] = h.groupby("code")["close"].shift(-1) / h["close"] - 1 # ← 익일 수익률
    ...
    for i in range(len(dates) - 1):
        t0_data = h[h["date"] == dates[i]][["code", "score", "next_return"]].dropna()
    ...
    print(f"[IC] Rolling({min(len(ic_list), window)}일) = {rolling_ic:.4f}")
    return rolling_ic
...
def normalize_score(df):
    ...
    df["score_norm"] = 50.0 if s_max - s_min < EPS else ((df["score"] - s_min) / (s_max - s_min) * 100).round(2)
    return df
...
def run_engine():
    ...
    cols = [c for c in ["code", "name", "close", "score_norm", "flow_z", "mom_z", "dart_z",
                        "foreign_net", "inst_net", "dart_score", "volume"] if c in df_top.columns]

"""
engine.py — v23.2 PLAN-A (FinanceDataReader 전환 / 로그인 불필요)
"""
...
# =========================
# 1. 유니버스 + 가격 (FDR — 로그인 불필요)
# =========================
def fetch_price_and_universe(date_str):
    try:
        # KOSPI 전체 종목 리스트
        kospi = fdr.StockListing("KOSPI")
        kospi["code"] = kospi["Code"].astype(str).str.zfill(6)
        kospi["name"] = kospi["Name"]
        print(f"[UNIVERSE] {len(kospi)}개 종목")

        # KRX 전체 시세 한번에
        date_fmt = datetime.strptime(date_str, "%Y%m%d").strftime("%Y-%m-%d")
        krx = fdr.DataReader("KRX", date_fmt, date_fmt)

        if krx is None or krx.empty:
            raise ValueError("KRX empty response")

        krx = krx.reset_index()

        # 컬럼 자동 감지
        code_col  = next((c for c in krx.columns if c in ["Code", "Symbol", "종목코드"]), None)
        close_col = next((c for c in krx.columns if c in ["Close", "종가"]), None)
        vol_col   = next((c for c in krx.columns if c in ["Volume", "거래량"]), None)

        print("[KRX 컬럼]", krx.columns.tolist())

        if code_col is None or close_col is None:
            raise ValueError(f"필수 컬럼 없음: {krx.columns.tolist()}")

        krx["code"]   = krx[code_col].astype(str).str.zfill(6)
        krx["close"]  = pd.to_numeric(krx[close_col], errors="coerce")
        krx["volume"] = pd.to_numeric(krx[vol_col], errors="coerce") if vol_col else 0

        result = kospi[["code", "name"]].merge(
            krx[["code", "close", "volume"]], on="code", how="inner"
        )
        result = result[result["close"] > 0].reset_index(drop=True)

        print(f"[PRICE] {len(result)}개 종목 가격 수집 완료")
        return result
...
def run_engine():
    ...
    # 1. 유니버스 + 가격
    df = fetch_price_and_universe(biz_day)
    if df.empty:

...
+ 버그픽스:
  [8] is_etf() / is_preferred() name NaN(float) 방어 처리
      str(name) 변환으로 AttributeError 완전 차단
...
class StockScorer:
    ...
    def is_preferred(self, name) -> bool:
        n = str(name)
        return (n.endswith("우") or
                "우B" in n or "우C" in n or
                "1우" in n or "2우" in n or "3우" in n)
    ...
    def top_n(self) -> list:
        ...
        for stock in self.stocks:
            code = str(stock.get("code", "")).zfill(6)
            name = stock.get("name", "")
            # [버그픽스8] NaN / 빈값 / "nan" 문자열 전부 차단
            if not code:
                continue
            name_str = str(name).strip()
            if not name_str or name_str.lower() == "nan":
                continue
            if self.is_etf(name_str):
                continue
            if self.is_preferred(name_str):
                continue

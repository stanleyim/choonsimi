"""
engine.py — v16 STABLE (INFINITE LOOP FIXED + FLOW FALLBACK)
"""
...
def compute_flow_data(df, date_str):
    ...
    if flow_df is not None and len(flow_df) > 0:
        ...
        df["foreign_net"] = 0
        df["inst_net"]    = 0
        print("[FLOW] fallback → 0 처리 (dart/mom으로 진행)")

...
def get_dart_financial(corp_code, dart_key):
    ...
    try:
        ...
        if equity > 0:
            ...
            debt_score = max(0, min(10, 10 - (debt_ratio / 40)))
        else:
            debt_score = 0

        if current_liab > 0:
            current_ratio = (current_assets / current_liab) * 100
            curr_score = min(20, max(0, current_ratio / 20))
        else:
            curr_score = 0
...
def score(m, l, r, rank, dart_score=0):
    ...
    # 핵심: DART 영향 현실화 (기존 과도함 수정)
    final = base * 0.75 + dart_score * 1.5

    return min(100, final)

# ---------- Main ----------
def main():
    print("[ENGINE v8.5 PRODUCTION START]")

    if not os.path.exists(CORP_MAP_PATH):
        raise RuntimeError("corp_map.json not found")
    ...
    if len(market) < 50:
        raise RuntimeError("market data insufficient")

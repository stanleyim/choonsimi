"""
engine.py — v23.1 PLAN-A (UNIVERSE HOTFIX)
로그인 불필요 / 무료 / 안정
"""
...
def fetch_price_and_universe(date_str):
    try:
        ...
        df["name"] = df["code"].apply(_safe_get_name)
        df = df[df["close"] > 0].reset_index(drop=True)

        print(f"[UNIVERSE+PRICE] {len(df)}개 종목 수집 완료")
        return df

"""
RegimeEngine v6.4.2-KIS-STABLE — OPERATION STABLE
─────────────────────────────────────────────────────
...
def select_top20(stocks, scorer):
    scored = []
    for s in stocks:
...
def run():
    ...
    result_data = select_top20(today_df, scorer)

    # KIS 가격 주입 + Fallback
    for bucket in ["top20", "entry_top5"]:
        for x in result_data[bucket]:
            code = x["code"]
    ...
    print("[DONE] v6.4.2-KIS-STABLE", today)

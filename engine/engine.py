...
수정:
  ...
  3. [FIX] 중복 컬럼으로 인한 InvalidIndexError 해결
"""
...
def load_news() -> dict:
    """
    news_fetch.run() 직접 호출해 실시간 뉴스 점수 반환.    실패 시 빈 dict (파이프라인 안 죽음).
    """
    try:
        import news_fetch
...
def run():
    ...
    # 2. [CRITICAL FIX] 이름 충돌 방지
    # final을 score로改名하기 전, 기존 score 컬럼을 제거하여 중복 방지
    ...
    if os.path.exists(HISTORY_PATH):
        ...
        # [FIX] 기존 히스토리에 중복 컬럼이 있을 경우를 대비해 정리

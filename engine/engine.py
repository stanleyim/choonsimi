"""
engine.py — Choonsimi FINAL PERFORMANCE ENGINE
- flow: 시장 방향성 (macro bias)
- momentum: 추세 (trend strength)
- news: 이벤트 (shock)
- volume: liquidity filter
- all features normalized into comparable scale
"""
...
def build_momentum(df):
    ...
    # 안정화 (폭 제거)
    df["momentum"] = np.tanh(df["momentum"])
...
def build_flow(df, flow_df):
    ...
    # normalization (중요)
    df["flow"] = np.tanh(flow_score / 1e12)
...
def compute_score(df):
    ...
    # normalize volume (중요 개선)
    df["volume"] = np.log1p(df["volume"])

    # FINAL ALPHA MODEL
    df["score"] = (
        df["flow"] * 0.40 +        # 가장 중요 (money flow)
        df["momentum"] * 0.30 +   # trend confirmation
        df["volume"] * 0.20 +     # liquidity filter
        df["news"] * 0.10         # event shock
    )

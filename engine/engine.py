"""
RegimeEngine v6.3.3 — FINAL SAFE (INDENTATION LOCKED + KEY ERROR DEFENSE)
──────────────────────────────────────────────────────────────────────────
✔ Indentation: 100% 4-space locked
✔ Syntax errors fixed (compute_regime / verify)
✔ save_signal_history: KeyError-safe
✔ NaN Safe Comparison (pd.notna)
✔ ROE / Debt Ratio 직접 노출
✔ Preferred Stock Filter (code 5/7/9)
──────────────────────────────────────────────────────────────────────────
"""
...
class StockScorer:
    ...
    def score(self, s):
        ...
        regime_factor = 1.05 if self.regime == "UPTREND" else (0.95 if self.regime == "DOWNTREND" else 1.0)
...
class RegimeEngine:
    ...
    def get_data_quality(self):
        try:
            ...
            return (
                "full"
                if df["date"].max() == datetime.now(KST).strftime("%Y-%m-%d")
                else "lagged"
            )
    ...
    def verify(self):
        try:
            ...
            price_map = {
                k: v for k, v in zip(hist["code"], hist["close"])

"""
fetch_dart.py — v2.3 (Name Field Added + FIXED VERSION)
"""
...
def get_corp_codes(key: str) -> dict:
    ...
    try:
        ...
        print(f"📦 corp_code 매핑 캐시 생성: {len(corp_map)}종목")
        return corp_map
...
def fetch_financial(key, corp_code, stock_code, year, reprt_code):
    ...
    try:
        ...
        print(f"❌ {stock_code} 재무데이터 조회 실패: {e}")
        return {}
...
def run():
    ...
    if os.path.exists(OUTPUT_PATH):
        try:
            ...
            if (
                ex.get("year") == year
                and ex.get("quarter") == quarter
                and ex.get("count", 0) >= 10
            ):
                print(f"ℹ️ {year} {quarter} 이미 수집됨 → 스킵")
                return
        except Exception as e:
            print(f"⚠️ 기존 output 로드 실패: {e}")
    ...
    if not corp_map:
        print("⚠️ corp_map 없음 → 스킵")
        return

    name_map = load_name_map()

    print(f"🎯 기준: {year} {quarter} | 대상: {len(target)}종목")
    ...
    print(f"[DART DONE] 성공={len(results)} 실패={error_cnt} 스킵={skip_cnt}")

"""
fetch_data.py — v4.3 (Option A Patch - FIXED)
─────────────────────────────────────
KIS API → 당일 보통주 데이터 수집
수정사항:
  ✅ FID_TRGT_CLS_CODE / EXLS_CLS_CODE 정확 수정 (critical fix)
  ✅ utf-8-sig 유지
  ✅ 토큰 6시간 캐시
  ✅ 실패 시 빈 CSV 생성
─────────────────────────────────────
...
def get_token() -> str:
    try:
        ...
        if token and (datetime.now(KST) - issued).seconds < 21600:
...
def fetch_volume_rank(token: str) -> list:
    ...
    # ✅ FIXED PARAMETER BLOCK (CRITICAL)
    params = {
    ...
    try:
        ...
        if data.get("rt_cd") != "0":
            print("[WARN] volume-rank API error:", data.get("msg1", ""))
            return []
        ...
        print(f"[DATA] volume-rank stocks: {len(rows)}")
        return rows

    except Exception as e:
        print("[WARN] volume-rank failed:", e)
        return []
...
def main():
    ...
    print("[DATA] start:", today)

    try:
        token = get_token()
    except Exception as e:
        print("[ERROR] token fail:", e)
        pd.DataFrame(columns=["date","code","name","close","volume","change_rate"]).to_csv(
    ...
    flow_rows = []
    for code in [c for c in get_flow_codes() if c not in known]:
        res = fetch_price(token, code)
        if res:
            flow_rows.append(res)
        time.sleep(DELAY)
    ...
    print(f"[DONE] saved {len(result)} stocks → {OUTPUT_CSV}")

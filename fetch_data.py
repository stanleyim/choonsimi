...
def fetch_daily_price(token, code):
    ...
    → output1: 종목 기본정보 (이름)
    → output2[0]: 가장 최근 거래일 확정 OHLCV
    장중/장후 상관없이 항상 동일한 확정값
    """
    if not token: return {}

    today     = datetime.now(KST).strftime("%Y%m%d")
    start_day = (datetime.now(KST) - timedelta(days=5)).strftime("%Y%m%d")
    ...
    for _ in range(MAX_RETRY):
        try:
            ...
            name  = info.get("hts_kor_isnm", "") if isinstance(info, dict) else ""
            ...
            # ── change_rate 계산 (v5.2.0 수정) ──────────
            # ① prdy_ctrt 우선 사용
            change_rate = safe_float(o.get("prdy_ctrt"))

            # ② prdy_ctrt = 0.0 → output2[1] 전일 종가로 직접 계산
            if change_rate == 0.0 and len(rows) > 1:
                prev_close = safe_int(rows[1].get("stck_clpr"))
                if prev_close > 0:
                    change_rate = round((close - prev_close) / prev_close * 100, 2)

            # ③ 여전히 0.0 → prdy_vrss 로 계산 (보조)
            if change_rate == 0.0:
                prdy_vrss = safe_float(o.get("prdy_vrss"))
                if prdy_vrss != 0.0:
                    prev_close = close - prdy_vrss
                    if prev_close > 0:
                        change_rate = round(prdy_vrss / prev_close * 100, 2)
...
def main():
    today = datetime.now(KST).strftime("%Y-%m-%d")
    print(f"[START] fetch_data v5.3.0  {today}")
    ...
    if not is_after_close:
        try:
            ...
            # 오늘 날짜로 저장된 잘못된 행 자동 정리
            had_today = (df_old["date"] == today).any()
            df_old    = df_old[df_old["date"] != today]

            if had_today:
                df_old.to_csv(OUTPUT_CSV, index=False, encoding="utf-8-sig")
                print(f"[CLEAN] 오늘({today}) 잘못 저장된 데이터 자동 삭제")
        except FileNotFoundError:
            pass

        print(f"[SKIP] 장마감 이전 ({now_kst.strftime('%H:%M')} KST) — "
              f"저장 스킵 (테스트 모드)")
        print(f"[INFO] KIS API 응답 정상 / engine은 기존 데이터로 작동")
        return

    # ── 장마감 후 실행: 정상 저장 ────────────────────────
    print(f"[TIME] 장마감 후 ({now_kst.strftime('%H:%M')} KST) — 저장 진행")

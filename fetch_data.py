...
def fetch_krx_supply(date_str):
    """KRX OpenAPI로 외국인/기관 순매수 + 주가 가져오기"""
    url = "https://opendata.krx.co.kr/api/stock/investorTradingBySecurities"
    params = {
        "key": KRX_API_KEY,
        "type": "json", 
...
def fetch_dart_scores(codes, date_str):
    """DART 공시 점수 - 기존 engine.py 로직 그대로 이관"""
    if not DART_API_KEY:
    ...
    try:
        ...
        positive = ["배당결정", "자사주취득", "영업이익증가", "수주계약", "흑자전환", "실적개선"]

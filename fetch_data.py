...
KRX_REFERER  = "http://data.krx.co.kr/"  # trailing slash 추가

# ★ KRX 실제 컬럼명 기반 후보 리스트 (2026년 기준)
KRX_COL_CANDIDATES = {
    "code":   ["종목코드", "티커", "ISU_SRT_CD", "Code", "code", "symbol"],
    "name":   ["종목명", "종목이름", "ISU_ABBRV", "Name", "name"],
    "foreign_net": [        # KRX 표준 컬럼명 (공백/언더바/괄호 변형 모두 포함)
...
def parse_krx_flow(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 디버그: 실제 컬럼명 출력 (파싱 실패 시 원인 파악용)
    if col_code is None or col_foreign is None or col_inst is None:
        print(f"  [KRX-DEBUG] 실제 컬럼: {df.columns.tolist()}")        if len(df) > 0:
        ...
        try:
            ...
            if not raw.empty:
                ...
                if not parsed.empty:
                    ...
                    print(f"  [KRX-OTP] {mkt_id} 파싱 성공: {len(parsed)}종목")
                else:
...
def fetch_fdr_cache():
    for date in trading_dates():
        ...
        try:
            ...
            print(f"  [1순위 FDR-CACHE] 성공: {date}  {len(df)}종목")            return add_placeholders(df, date), date, "fdr_cache"
